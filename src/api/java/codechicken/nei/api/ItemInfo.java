...
/**
 * This is an internal class for storing information about items, to be accessed
 * by the API
 */
public class ItemInfo {
	...
	private static void addIDDumps() {
		...
		API.addOption(new RegistryDumper<Block>("tools.dump.block") {
			...
			public String[] dump(Block item, int id, String name) {
				return new String[] { name, Integer.toString(id), Boolean.toString(Item.getItemFromBlock(item) != null),
	...
	private static void addEntityEgg(Class<?> entity, int i, int j) {
		try {
			Field field = EntityList.class.getDeclaredField("classToIDMapping");
			field.setAccessible(true);
			Map map = (Map) field.get(null);
			int id = (Integer) map.get(entity);
	...
	public static String getSearchName(ItemStack stack) {
		...
		if (s == null) {
			s = EnumChatFormatting.getTextWithoutFormattingCodes(
