...
public class ItemBathtub extends Item {
	public ItemBathtub() {
		this.setUnlocalizedName("bathtub");
		this.setTextureName("digcraft:bathtub");
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int face,
		...
		case 0:
			if (this.trySetBlock(world, x, y - 1, z, 0, face, player, stack)
				...
				world.setBlock(x, y - 1, z, BlockLoader.bathtub, 0, 2);
				world.setBlock(x - 1, y - 1, z, BlockLoader.bathtub, 8, 2);
			} else {
				return false;
			}
			break;
		...
		case 2:
			if (this.trySetBlock(world, x, y, z - 1, 0, face, player, stack)
				...
				world.setBlock(x, y, z - 1, BlockLoader.bathtub, 0, 2);
				world.setBlock(x - 1, y, z - 1, BlockLoader.bathtub, 8, 2);
			} else {
				return false;
			}
			break;
		case 3:
			if (this.trySetBlock(world, x + 1, y, z + 1, 10, face, player, stack)
				...
				world.setBlock(x + 1, y, z + 1, BlockLoader.bathtub, 9, 2);
				world.setBlock(x, y, z + 1, BlockLoader.bathtub, 1, 2);
			} else {
				return false;
			}
			break;
		case 4:
			if (this.trySetBlock(world, x - 1, y, z + 1, 10, face, player, stack)
				...
				world.setBlock(x - 1, y, z + 1, BlockLoader.bathtub, 10, 2);
				world.setBlock(x - 1, y, z, BlockLoader.bathtub, 2, 2);
			} else {
				return false;
			}
			break;
		case 5:
			if (this.trySetBlock(world, x + 1, y, z, 0, face, player, stack)
					&& this.trySetBlock(world, x + 1, y, z - 1, 11, face, player, stack)) {
				world.setBlock(x + 1, y, z, BlockLoader.bathtub, 3, 2);
				world.setBlock(x + 1, y, z - 1, BlockLoader.bathtub, 11, 2);
			} else {
				return false;
			}
			break;
