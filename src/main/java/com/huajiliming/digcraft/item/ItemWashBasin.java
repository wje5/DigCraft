...
public class ItemWashBasin extends Item {
	public ItemWashBasin() {
		this.setUnlocalizedName("washBasin");
		this.setTextureName("digcraft:washBasin");
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int face,
		...
		case 0:
			if (this.trySetBlock(world, x, y - 1, z, 0, face, player, stack)
				...
				world.setBlock(x, y - 1, z, BlockLoader.washBasin, 0, 2);
				world.setBlock(x - 1, y - 1, z, BlockLoader.washBasin, 8, 2);
			} else {
				return false;
			}
			break;
		...
		case 2:
			if (this.trySetBlock(world, x, y, z - 1, 0, face, player, stack)
				...
				world.setBlock(x, y, z - 1, BlockLoader.washBasin, 0, 2);
				world.setBlock(x - 1, y, z - 1, BlockLoader.washBasin, 8, 2);
			} else {
				return false;
			}
			break;
		case 3:
			if (this.trySetBlock(world, x + 1, y, z + 1, 10, face, player, stack)
				...
				world.setBlock(x + 1, y, z + 1, BlockLoader.washBasin, 9, 2);
				world.setBlock(x, y, z + 1, BlockLoader.washBasin, 1, 2);
			} else {
				return false;
			}
			break;
		case 4:
			if (this.trySetBlock(world, x - 1, y, z + 1, 10, face, player, stack)
				...
				world.setBlock(x - 1, y, z + 1, BlockLoader.washBasin, 10, 2);
				world.setBlock(x - 1, y, z, BlockLoader.washBasin, 2, 2);
			} else {
				return false;
			}
			break;
		case 5:
			if (this.trySetBlock(world, x + 1, y, z, 0, face, player, stack)
					&& this.trySetBlock(world, x + 1, y, z - 1, 11, face, player, stack)) {
				world.setBlock(x + 1, y, z, BlockLoader.washBasin, 3, 2);
				world.setBlock(x + 1, y, z - 1, BlockLoader.washBasin, 11, 2);
			} else {
				return false;
			}
			break;
